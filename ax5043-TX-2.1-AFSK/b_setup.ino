...
void setup() {
  ...
  // Initial check - Get Silicon Revision data
  ...
  // display reg value
  Serial.print("Initial check - Get Silicon Revision data(Should be 0x51)");
  Serial.print("\n"); //newline
  ...
  // Reset the AX5043, (SEL Pin 14 on AX5043)
  digitalWrite(slaveSelectPin, HIGH);
  delay (2); // 2mS delay
  ...
  //uint16_t configure1[2][10]  = { { 0xF10, 0xF11, 0xF35, 0x184, 0xF34, 0x032, 0x34, 0x35, 0x36, 0x37, }, { 0x03, 0x07, 0x10, 0x00, 0x28, 0x34, 0x01, 0xB0, 0x00, 0x00,  } }; // configuration values into array for 75 MHz
  ...
  Serial.print("Frequency set to 458.75 MHz");
  Serial.print("\n"); //newline
  ...
  Serial.print("Setting frequency finished");
  Serial.print("\n"); //newline

  // Start Autoranging
  regAddr = 0x33; // PLLRANGINA register
  regVal = 0x10;  // Set the RNG START pin - Bit 4
  writeRegister(regAddr, regVal); // Function call
  Serial.print("PLLRANGINA register set to autorange");
  Serial.print("\n"); //newline

  delay(10000); // wait for 10 sec for auto ranging
  ...
  if ( value & testbit4 )      // test bit shown. Test if both are true.
    //  if (bitRead(regVal, 5) = HIGH ) // test bit shown. Test if both are true.
  {
    // Auto ranging failed - Bit 4 is still set
    Serial.print("Auto ranging failed");
    Serial.print("\n"); //newline
  }
  else
  {
    // Auto ranging successful- Bit 4 is not set
    Serial.print("\n"); //newline
